...
class ContractConstructor {
  ...
  getCode() {
    ...
    return this.getToken()
      ...
      .then(response => {
        migration = response;
        return axios.get("contract-builder/template/deploy.sh", {
          responseType: "blob"
        });
      })
      .then(response => {
        deploy = response.data;
        return axios.get("contract-builder/template/package-lock.json", {
          responseType: "blob"
        });
      })
      .then(response => {
        packageLock = response.data;
        return axios.get("contract-builder/template/package.json", {
          responseType: "blob"
        });
      })
      .then(response => {
        packageFile = response.data;
        return axios.get("contract-builder/template/truffle-config.js", {
          responseType: "blob"
        });
      })
      .then(response => {
        truffleConfig = response.data;
        return axios.get("contract-builder/template/truffle.js", {
          responseType: "blob"
        });
      })
      .then(response => {
        truffle = response.data;
        return axios.get("contract-builder/template/contracts/Migrations.sol", {
          responseType: "blob"
        });
      })
      .then(response => {
        ...
        return axios.get(
          "contract-builder/template/scripts/setDefaultAccount.js",
          { responseType: "blob" }
        );
      })
      .then(response => {
        ...
        return axios.get(
          "contract-builder/template/scripts/createTestAccount.js",
          { responseType: "blob" }
        );
      })
      .then(response => {
        ...
        return axios.get(
          "contract-builder/template/scripts/createRealAccount.js",
          { responseType: "blob" }
        );
      })
      .then(response => {
  ...
  getToken() {
    return axios
      .get("contract-builder/template/contracts/Token.sol")
      .then(response => {
  ...
  getCrowdsale() {
    return axios
      .get("contract-builder/template/contracts/Crowdsale.sol")
      .then(response => {
        return response.data
          .replace(/{{symbol}}/g, this.symbol)
  ...
  getMigration() {
    return axios
      .get("contract-builder/template/migrations/1_initial_migration.js")
      .then(response => {
        return response.data
          .replace(/{{symbol}}/g, this.symbol)
          ...
          .replace(
            /{{migrationConstructorParams}}/g,
            this.getMigrationConstructorParams()
          );
      });
  }
  ...
  getMigrationConstants() {
    let result = "";
    if (this.contractOptions.includes(ContractOptions.Timed)) {
      const options = this.contractParams[
        this.contractOptions.indexOf(ContractOptions.Timed)
      ];
      result = `${result}\n    const openingTime = (new Date(${
        options[0] ? `'${options[0]}'` : "2024, 04, 07"
      })).getTime/1000\n    const closingTime = (new Date(${
        options[1] ? `'${options[1]}'` : "2025, 04, 07"
      })).getTime/1000`;
    }
    if (this.contractOptions.includes(ContractOptions.Refundable)) {
      const options = this.contractParams[
        this.contractOptions.indexOf(ContractOptions.Refundable)
      ];
      result = `${result}\n    const goal = ${options[0] || 1000}`;
    }
    if (this.contractOptions.includes(ContractOptions.IncreasingPrice)) {
      const options = this.contractParams[
        this.contractOptions.indexOf(ContractOptions.IncreasingPrice)
      ];
    ...
    if (this.contractOptions.includes(ContractOptions.Capped)) {
      const options = this.contractParams[
        this.contractOptions.indexOf(ContractOptions.Capped)
      ];
